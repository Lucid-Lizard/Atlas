...
namespace Atlas.Content.Items.Celestial
{
    public class PongPing : ModItem
    {
        public override void SetDefaults()
        {
            ...
            Item.shoot = ModContent.ProjectileType<VortexBall>();
            Item.shootSpeed = 18f;
        ...
        {
            if (player.altFunctionUse == 2 && player.ownedProjectileCounts[Item.shoot] >= 10)
            {
                return false;
            }
        ...
        {
            Projectile.NewProjectile(source, position, new Vector2(0, -3), type, damage, knockback);
        ...
        public override void UseItemHitbox(Player player, ref Rectangle hitbox, ref bool noHitbox)
        {
            ...
            for (int i = 0; i < Main.maxProjectiles; i++)
            {
                ...
                if (p.active && p.ModProjectile is PongBall && Collision.CheckAABBvLineCollision(p.position, p.Hitbox.Size(), start, end) && player.altFunctionUse < 2)
                {
                    ...
                    Dust.NewDust(p.Center, 1, 1, DustID.Vortex, dust1.X, dust1.Y);
                    Dust.NewDust(p.Center, 1, 1, DustID.Vortex, dust2.X, dust2.Y);
